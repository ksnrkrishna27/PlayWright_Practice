...
describe("Order History flows", () => {
  test(
    "Order History flow1",
    { tag: ["@flow1", "@Order-Historyflows"] },
    async () => {
      const new_page: Page = await login("abcz1@gmail.com", "abc123");

      const order_history_locator: Locator = await new_page.locator(
        '//*[@id="column-right"]/div/a[6]'
      );
      ...
      await new_page.screenshot({
        path: "./Screenshots/Order_History_flow1.png",
      });
  ...
  test(
    "Order History flow2",
    { tag: ["@flow2", "@Order-Historyflows"] },
    async () => {
      const new_page: Page = await login("abcz1@gmail.com", "abc123");

      const My_Account_locator: Locator = await new_page.locator(
      ...
      await new_page.screenshot({
        path: "./Screenshots/Order_History_flow2.png",
      });
